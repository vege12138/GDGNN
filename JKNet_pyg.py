...
def train():
    ...
    for epoch in range(opt.epochs):
        ...
        out = model(data, normalized_adjacency_matrix)
        loss = criterion(out[data.train_mask], data.ndata['label'][data.train_mask])
...
def valid():
    ...
    with torch.no_grad():
        out = model(data, normalized_adjacency_matrix)
        loss = criterion(out[data.val_mask], data.ndata['label'][data.val_mask])
...
def test():
   # model.load_state_dict(torch.load('best_model.pth'))
    model.eval()
    out = model(data, normalized_adjacency_matrix)
    loss = criterion(out[data.test_mask], data.ndata['label'][data.test_mask])
...
if __name__ == '__main__':
    ...
    for name in range(5, 8):
        ...
        #
        # filename = f"exp\\{opt.name}\\{opt.name}_repeat{repeat_times}_{max_mean_acc}.xlsx"
        #
        # # 获取文件夹路径
